...
#### 2.3 Weigh Particles
This step in the code is for weighing each particle based on its likelyhood of being in the true pose position. After this step, each particle should have a value that represents the probability that it will be kept for the next step, resampling. To begin, the LIDAR message was used to create an array of points in front of the robot, each symbolizing an object in the robot's view. They were defined by an angle (the LIDAR message index) and a distance (the lidar message). Doing this kept the points in the base frame, meaning they could be easily tranformed onto each particle. For each particle, the 'ghost' readings were redefined in the map frame using the particle's position. We then utalized the occupancy field function to find the closest object to each ghost point. Theoretically, a particle in the exact pose position would recieve a distance of 0 for each ghost point, as they would be on top of the actual map obstacle.
...
Step two was to multiply these percenteges by each other. For each particle, an array of these percenteges was multiplied together. At this stage, each particle had a number representing its 
